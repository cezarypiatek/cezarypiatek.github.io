...
As you can see, our `CustomWebApplicationFactory` allows quite easily for replacing services in DI container, but there are a few things that I don't like here:
- There is certain amount of code that you need to always repeat for every mocked service. It degrades test code readability and distracts our attention from what's really important here. 
- We need to know the original component lifecycle, which smells like a information leak. The component lifecycle is not important from the test case perspective, so it should't be visible here.
...
```
Now our test looks a little bit more slim:
...
This is definitively progress in the right direction, but there's still one problem that might affects test mailability.
Mocking explicitly specific interfaces in our test case code makes test tightly coupled to the implementation details. This is not good because every time when we refactor application code (without changing its behavior) we will be forced to adjust test code to the new implementation. A good example for that could be when we decide that repository pattern doesn't play well with our architecture and we want to replace with different solution. This could turn our internal structure upside down. Ideally, after every refactor, we would like to run existing test suit (without changing it) to verify that the expected behavior was preserved. The same problem could appear when the data source will change - for example our app will need to load a given entities from new GRPC service instead of from old database. Changing `IYDataRepository` to `ISampleGrpcService` will required changing code of all test cases that mocks `IYDataRepository` interface.

We can try to limit the scope of changes by abstracting code responsible for mocking specific interfaces. This can be done for example with test data builder approach:
...
> Ideally, the data builder methods should not accept as parameters application internal structure but rather a custom data structures that contain only a subset of attributes that matters from test case perspective. This can be done by creating a custom data structure for that purpose or by listing necessary attributes as builder method parameters. Builder method should be responsible for translating it into app internal structures. In case of any app internal changes, we will need to adjust only builder methods - test case code will be intact.


To use the new approach with data builder, we need to modify our `CustomWebApplicationFactory` to accept as constructor parameter a lambda that customize the `TestDataBuilder` instead of `IServiceCollection`:
...
Using new version of `TestDataBuilder` we can prepare test data in a very concise, readable, and maintainable way as follows:
