...
__UPDATE 2021-02-09:__ Sometimes there's a need to enrich our MsTeams message with data that are not available through the build variables like build status or build url. This can be easily achieve by calling a dedicated TeamCity REST API endpoint and storing the result in a variable that can be later referenced inside the message's title or body expression:
...
Example usage in `Title` field in build step created based on our metarunner:
