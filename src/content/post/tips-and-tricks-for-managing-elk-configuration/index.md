...
A few months ago I've published "[Demystifying ELK stack](/post/demystifying-elk-stack/)" article that summarizes my knowledge about setting up and configuring the system for collecting, processing and presenting logs that is based on Filebeat, Logstash, Kibana, and Elasticsearch. Since then I've learned a few new DevOps things which help me and my teammates to work more effectively with ELK. I think it's worth to share them.
...
Currently, I've 10 internal environments for different purposes (manual testing, automated UI testing, load testing, presentations for different customers etc.). Each environment consists of two web applications and each of them produces 2 log files (diagnostic and performance data). That gives me `10 * 2 * 2 = 40` log files and for any of them, I have to configure a separated prospector. I cannot use single prospector with a wildcard in the `path` attribute because there is a need to add additional metadata such as environment name, app name and log type (attributes defined in the `fields` node).  However, some of the attributes are the same for every prospector which cause a massive configuration duplication and make it harder to modify this common values. I was even thinking about preparing some kind of template for prospector configuration with some custom `PowerShell` script that could facilitate creating a config for all environment. Instead of rushing to develop in-house solution I started from browsing `YAML` specification and I've found [Merge Key Language-Independent Type](http://yaml.org/type/merge.html) which seems to be a response to my problem. The ``!!merge`` feature allows to define and reuse keys of the map.  In the following example, I've defined a common configuration for prospectors as `PROSPECTOR_COMMON_OPTIONS` and reference it in any prospector configuration with `<< : *` notation. 
...
Every time when I changed `Logstash` or `Filebeat` configuration I had to log in to the appropriate server, replace the old config with the new one, restart the service and examine the service log file if the whole operation finish with success. If something failed I need to correct the config file and repeat the whole mantra. It was a very tedious process and nobody in the team besides me know to how to do it. I even wrote the whole instruction down but the number of steps or the need to log into `Linux` server repeals others. A better than creating manual instruction is to automate the process. The easiest part was to create the script that update `Filebeat` configuration because it resides on the Windows server:
...
If you are working on `Ubuntu` then you can check your current version with `lsb_release -a` command. If everything went well you should be able to enter the `PowerShell` console with `pwsh` command. 
...
Despise all my concerns the installation went pretty smoothly (I only needed to adjust the version of package responsible for `SSL`) and I was able to invoke remotely command on the Linux server from my Windows workstation with `Invoke-Command` cmdlet.  Now I was able to easily automate the process of updating `Logstash` config:
...
I've enriched my script with `Watch-File` function that forward `Logstash` logs and blocks the process until the restart finish. Thanks to that we have live stream what is going on during the restart. Now the Logstash config update can be performed with the command:
